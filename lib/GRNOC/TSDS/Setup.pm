...
sub upgrade{
	...
	print "\n Execute 'hostname' command and enter result below";
	...
	for (my $i=1; $i <= $no_of_shard; $i++) {
		...
		print "\n Adding shard with hostname $hostname and port $server_port \n";
		if(system("mongo --eval \"printjson(sh.addShard('$hostname:$server_port'));\" ")!=0){
			print "\n Error occured while adding shard with port $server_port to mongo";
			exit;
		}
	}
